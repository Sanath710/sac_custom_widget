...
;(function () {
  ...
  class customPieChart extends HTMLElement {
    ...
    onCustomWidgetAfterUpdate(changedProperties) {
      ...
      if("colorPallate" in changedProperties) {
        this.render(this._myDataBinding, this._props)
      }
    }

    async setTableWidget(table) {
      console.log("Table");
      this.myTable = await table;
      this.myTable.getDataSource().setDimensionFilter('City','Dallas');
      console.log(this.myTable.getDataSource().getResultSet());
    }
    ...
    async render (myDataBinding, props) {
      ...
      for(var i = 0; i < myDataBinding["data"].length; i++) {
        ...
        if(this.colorPallate && i < this.colorPallate.length) {
          obj["color"] = this.colorPallate[i];
        }
        data.push(obj);
      }
