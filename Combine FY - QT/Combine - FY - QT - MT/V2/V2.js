        ...
        class CustomTable extends HTMLElement {
            ...
            showTotal(callFrom) {
                ...
                if(callFrom == "MT") {
                    ...
                    // console.log(this._dataTableObj.columns(".vspy"), varCols, perCols)
                }
                ...
                //             sum = parseFloat(sum).toFixed(no_of_decimalPlaces).toString()+"%"
                //         }
                //         this._dataTableObj.cell(key, indices[i]).data(sum)
                //     }
                // }
...
//  ------------------------------ TO BE UNCOMMENTED ----------------------
            ...
            async render_MT() {
                ...
                for(var i = 0; i < this._tableColumnNames.length; i++) {
                    if(this._tableColumnNames[i]["className"] == "varCol") {
                        varColIndices.add(i);
                    }
                }
...
//  ------------------------------ TO BE UNCOMMENTED ----------------------
                ...
                for(const [k, v] of Object.entries(masterObj)) {
                    ...
                    for(var kk = 0, optIdx = 0; kk < sliced.length; kk++) {
                        ...
                        if(flag && masterObj[k]["DropDownFieldName"].length == 1) {
                            var empty_select_html = `<select id='${kk}' class='row_level_select row_level_select_${kk}_${this._dataTableObj.rows().count()}' onchange='updateRow(this, ${this._dimensions.length}, ${this._measureOrder.length}, ${this._excludeHeaders.length}, "${finalRow[0]}_#_${finalRow[1]}", ${fixedScenarioAt + 1 + this._colOrder.length * varianceCnt}, ${this._colOrder.length * varianceCnt})'>option selected>${masterObj[k]["DropDownFieldName"][0]}</option></select>`;
                            caughtDropDownsAt.add(0)
                            sliced[kk] = empty_select_html;
                        } else {
                            var select_html = `<select id='${kk}' class='row_level_select row_level_select_${kk}_${this._dataTableObj.rows().count()}' onchange='updateRow(this, ${this._dimensions.length}, ${this._measureOrder.length}, ${this._excludeHeaders.length}, "${finalRow[0]}_#_${finalRow[1]}", ${fixedScenarioAt + 1 + this._colOrder.length * varianceCnt}, ${this._colOrder.length * varianceCnt})'>`;
                            var options = "";
                            for(var p = 0; p < dropdownArr.length; p++) {
                                if(dropdownSel.includes(dropdownArr[p])) {
                                    caughtDropDownsAt.add(p)
