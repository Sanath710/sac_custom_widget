            ...
            showTotal() {
                ...
                for(var i = 0; i < this._dataTableObj.rows()[0].length; i++) {
                    ...
                    if(considerThisRow.length && considerThisRow[0] == "group") {
                        rowID = subsetRowID;
                        groupRowMapping[rowID] = [];
                    } 
                    if(rowID) {
                        groupRowMapping[rowID].push(subsetRowID)
                    }
                }

                // console.log("Row ID : Subset Group Mapping", groupRowMapping)

                for(var [key, val] of Object.entries(groupRowMapping)) {
                    for(var i = 0; i < indices.length; i++) {
                        var sum = 0;
                        if(indices[i] != -1) {
                ...
                function updateTotalOnRowUpdate(updateFrom, no_of_mes, updateFromRowID) {
                    ...
                    for(var i = updateFrom; i < updateFrom + no_of_mes; i++) {
                        ...
                        for(var j = 1; j < d.length; j++) {
                            if(!Object.keys(groupRowMapping).includes(j.toString())){
                        ...
                        for(var k = updateFrom; k < updateFrom + no_of_mes; k++) {
                            ...
                            if(!isNaN(dc)) {
                                tbl.cell(parseInt(parentID), k).data(parseFloat(subsetSum).toFixed(2))
                            }
                        }
