    ...
    ; (function (isLibAvail = false) {
        ...
        class CustomTable extends HTMLElement {
            ...
            async render() {
                ...
                // TRIM LOGIC
                // table_cols = table_cols.slice(0, this._dimensions.length - this._excludeHeaders.length + ((this._measureOrder.length + 1) * 3) - 1)
                ...
                for(var i = 0; i < this._scenarioOrder.length - 2; i++) {
                    if(this._fixedScenario.includes(this._scenarioOrder[i])) {
                        ...
                        `<th class='top-header' colspan='${this._measureOrder.length + 1}'>
                            <select id='${i}' onchange='updateColumns(this, ${this._dimensions.length}, ${this._excludeHeaders.length}, ${this._measureOrder.length}, ${this._scenarioOrder.length - this._fixedScenario.length})'>`;
                            var opts = ""
                ...
                function updateColumns(state, no_of_dimens, no_of_excludes, no_of_mes, scenario_len) {
                    ...
                    for(var i = 0; i < selectData.length; i++) {
                        ...
                        // console.log(masterRows[i].slice(sliceTill - 1, sliceTill + no_of_mes))
                        ...
                        for(var j = 0, cntr = hMap[state.id - 1]; j < mr.length; j++) {
                            if(j == 0) {
                                // console.log(updatedColData[hMap[state.id - 1]], "---", updatedColData[hMap[state.id - 1]].id)
                                // console.log("---", updatedColData[hMap[state.id - 1]],"+++", mr[j])
                            //     if(updatedColData[hMap[state.id - 1]].id == undefined) {
                            //         console.log(updatedColData,"****",updatedColData[hMap[state.id - 1]])
                                    // updatedColData[cntr] = updatedColData[hMap[state.id - 1]].split("onchange")[0]+" onchange"+mr[j].split("onchange")[1]
                            //     } else {
                ...
                for(var i = 0, prev_key = ""; i < this._resultSet.length;) {
                    ...
                    if(key == prev_key) {
                        ...
                        for(var k = 0, optIdx = 0; k < sliced.length; k++) {
                            var select_html = `<select id='${k}' onchange='updateRow(this, ${this._dimensions.length}, ${this._measureOrder.length}, ${this._excludeHeaders.length}, "${fixRows[0]}_#_${fixRows[1]}")'>`;
                            var options = "";
                            for(var p = 0; p < dateNameArr.length; p++) {
                                if(optIdx == p) {
                                    // value='${JSON.stringify(fixRows)}'
                                    // console.log(fixRows.join("_#_"))
